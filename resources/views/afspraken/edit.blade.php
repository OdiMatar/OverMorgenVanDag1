...
<main class="page-shell afspraak-form-page">
    ...
    <section class="afspraak-form-card" aria-label="Afspraak wijzigen">
        <form method="POST" action="{{ route('afspraken.update', $afspraak->Id) }}">
            ...
            <div class="afspraak-form-grid">
                ...
                <label for="behandeling_id">
                    ...
                    <select id="behandeling_id" name="behandeling_id" class="{{ $errors->has('behandeling_id') ? 'is-invalid' : '' }}" required>
                        @foreach ($behandelingen as $behandeling)
                            <option value="{{ $behandeling->Id }}" {{ old('behandeling_id', $afspraak->medewerkerPerBehandeling->BehandelingId) == $behandeling->Id ? 'selected' : '' }}>
                ...
                <!-- 2. Medewerker kiezen (Bug US2 Fix: Specialisatie tonen) -->
                <label for="medewerker_id">
                    ...
                    <select id="medewerker_id" name="medewerker_id" class="{{ $errors->has('medewerker_id') ? 'is-invalid' : '' }}" required>
                        @foreach ($medewerkers as $medewerker)
                            <option value="{{ $medewerker->Id }}" {{ old('medewerker_id', $afspraak->medewerkerPerBehandeling->MedewerkerId) == $medewerker->Id ? 'selected' : '' }}>
                ...
                <label for="datum">
                    ...
                    <select id="datum" name="datum" class="{{ $errors->has('datum') ? 'is-invalid' : '' }}" required>
                        @foreach ($beschikbareDatums as $datum)
                            <option value="{{ $datum }}" {{ old('datum', $afspraak->Datum) == $datum ? 'selected' : '' }}>
                ...
                <label for="starttijd">
                    ...
                    <select id="starttijd" name="starttijd" class="{{ $errors->has('starttijd') ? 'is-invalid' : '' }}" required>
                        @foreach ($tijdstippen as $tijd)
                            <option value="{{ $tijd }}" {{ old('starttijd', $afspraak->Starttijd) == $tijd ? 'selected' : '' }}>
            ...
            <div class="afspraak-actions">
                ...
                <button type="submit" class="afspraak-btn-submit">Wijziging opslaan</button>
            </div>
        </form>
    </section>
</main>
